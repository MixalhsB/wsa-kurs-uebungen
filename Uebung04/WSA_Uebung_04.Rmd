...
10. Konvertiere den Wert von `e` von einem Wahrheits- in einen Textwert, indem du einen Befehl der Form `as.<Ziel-Datentyp>(…)` verwendest. Weise das Ergebnis der Konversion wieder der Variablen `f` zu. Überprüfe anschließend den Datentyp von `e` mithilfe des `class(…)`-Befehls.
...
9. Erstelle einen Datenrahmen `potsdamer_bezirke_mehr_als_3_ortsteile`, der genau so strukturiert ist wie `potsdam`, aber lediglich die Zeilen enthält, für die in Spalte `AnzahlOrtsteil` ein Zahlenwert vorliegt, der größer als 3 ist.\newline
